...
void main() {
  ...
  group('isExpired', () {
    ...
    test('return true when current date is after expiry date', () {
      when(mockPref.retrieveString('key_expiry_date')).thenReturn(tExpiryDate1);
      final result = cache.isExpired(tKey);
      verify(mockPref.retrieveString('key_expiry_date'));
      expect(result, true);
    });
    test('return false when current date is before expiry date', () {
      when(mockPref.retrieveString('key_expiry_date')).thenReturn(tExpiryDate2);
      final result = cache.isExpired(tKey);
      verify(mockPref.retrieveString('key_expiry_date'));
    ...
    test('return true when expiry date is null', () {
      when(mockPref.retrieveString('key_expiry_date')).thenReturn(null);
      final result = cache.isExpired(tKey);
      verify(mockPref.retrieveString('key_expiry_date'));
      expect(result, true);
    });
